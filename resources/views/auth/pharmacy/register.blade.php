...
<head>
    ...
    <title>Star Admin2 - Pharmacy Registration</title>
    <!-- plugins:css -->
            ...
            <div class="row w-100 mx-0">
                <div class="col-lg-4 mx-auto">
                    <div class="auth-form-light text-left py-5 px-4 px-sm-5">
                        ...
                        <form class="pt-3" method="POST" action="{{ route('pharmacy.register') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control form-control-lg @error('address') is-invalid @enderror" id="exampleInputAddress1" name="address" placeholder="Address" value="{{ old('address') }}" required>
                                @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" class="form-control form-control-lg @error('license_image') is-invalid @enderror" id="license_image" name="license_image" required>
                                @error('license_image')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-4">
                                <div class="form-check">
                                    <label class="form-check-label text-muted">
                                        <input type="checkbox" class="form-check-input @error('terms') is-invalid @enderror" required> I agree to all Terms & Conditions
                                    </label>
                                    @error('terms')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
